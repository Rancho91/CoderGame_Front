...
const Home = () => {
  ...
  useEffect(() => {
    ...
    videogames().then((response) => {
      ...
      const sliderImage = imagenSlader(response.Videogames, 5);
      setArrSlader(sliderImage);
      const imageFooter = imagenSlader(response.Videogames, 3);
      setArrGameFooter(imageFooter);
    });
  }, []);
  ...
  return (
    <div className={`container ${styles.home}`}>
      ...
      <div className={`row d-flex justify-content-center cardInner ${styles.cardInner} cardContainer`}>
        {arrGamesFooter?.map((game) => (
          <div className={`col-lg-4 col-md-6 col-6 ${styles.card}`}>
            <Card game={game} showPriceAndBuy={false} />
          </div>
        ))}
      </div>
